...
def get_historic_data(grouping, period, timeframe, from_start_of_year=False):
  ...
  for row in db_data:
    ret_data.append(row[1] / 1000 * 3600)  # convert 10' avg W/m2    to    kWh/m2
...
def plot_graph(output_file, data_tuple, plot_title):
  ...
  trend_data = data_tuple[1]
  data_lbls, trend_data, dummy_data = kam41.build_arrays44(data_lbls, trend_data, trend_data)
  ...
  for idx in range(0, len(data_lbls[0])):
    ax1.bar(tick_pos + (idx * bar_width), trend_data[idx],
            width=bar_width,
...
def main():
  ...
  if OPTION in ['-m', '-M', '-y', '-Y', '-a', '-A']:
    plot_graph('/tmp/kamstrupd/site/img/sun_vs_month.png',
               fetch_last_months(),
               f"Zonnestraling per maand afgelopen jaren ({dt.now().strftime('%d-%m-%Y %H:%M:%S')})"
               )
