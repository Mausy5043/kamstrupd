...
class Zappi():
    ...
    def get_server_url(self):
        ...
        myenergi_response = requests.get(self.myenergi_base_url,
                                         auth=HTTPDigestAuth(self.hub_serial,
                                                             self.hub_password
                                                             )
                                         )
        if 'X_MYENERGI-asn' in myenergi_response.headers:
            self.myenergi_base_url = 'https://' + myenergi_response.headers['X_MYENERGI-asn']
        else:
    ...
    def get_status(self):
        ...
        for i in range(3):
            try:
                ...
                response = requests.get(theURL, headers=hdrs, auth=HTTPDigestAuth(self.hub_serial, self.hub_password), timeout=10)
            except:
